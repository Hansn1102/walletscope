...
app.get("/api/scan/:address", async (req, res) => {
  ...
  try {
    ...
    for (const pos of positions) {
      ...
      const iconUrl =
        attr.fungible_info?.icon?.url ?? null;
    ...
    // Filter out zero-value spam tokens, keep top holdings
    const validTokens = tokens.filter((t) => t.value > 0);
    for (const t of validTokens) {
      t.pct =
        totalValueUsd > 0
          ? Math.round((t.value / totalValueUsd) * 10000) / 100
          : 0;
    }
    ...
    if (tier === "behavior") {
      return res.json({ portfolio, behavior });
    }
        ...
        : totalValueUsd >= 10000
          ...
          : totalValueUsd >= 1000
            ...
            : totalValueUsd >= 100
              ...
              : 0;
    const s2 =
      validTokens.length >= 10 && topPct < 50
        ? 20
        : validTokens.length >= 5 && topPct < 70
          ? 15
          : validTokens.length >= 3
            ? 10
            : 5;
    const s3 =
      avgPerDay >= 3
        ...
        : avgPerDay >= 1
          ...
          : txCount >= 5
            ...
            : txCount >= 1
              ...
              : 0;
    const s4 =
      interactions.size >= 20 && tradingStyle !== "Active Trader"
        ? 20
...
app.listen(PORT, () => {
  console.log(`WALLETSCOPE dashboard running at http://localhost:${PORT}`);
});
